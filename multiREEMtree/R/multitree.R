...
#'  \item fitted - A matrix containing the fitted values for the dataset.
#'  \item residuals - A matrix containing the residuals of the fitted model.
#'}
#'
#'
#' @import mvpart
  ...
  #
{
  ...
  Fitted <- tree$frame[as.character(tree$where), ]$yval2
  
  colnames(Fitted) <- ResponseVariables
  
  Residuals <- OriginalResponse - Fitted
  ...
  result <- list(Tree = tree, Formula = formula, ResponseVariables = ResponseVariables, 
                 ...
                 stand.y = stand.y, fitted = Fitted, residuals = Residuals)
