#' Extract the terminal nodes (leaves) of a multiREEMtree object
#'
#' Extract the terminal nodes (leaves) of a multiREEMtree object, and provide summary statistics for
...
#' \itemize{
#'  \item terminal_nodes - a character vector showing all the terminal nodes.
#'  \item terminal_nodes_table - a table showing the number of observations in each terminal node.
#'  \item terminal_nodes_where - a vector showing which terminal node each observation belongs to.
...
getTerminal <- function(object){
    ...
    if(class(object)=="multiREEMtree"){
      ...
      stop("The object should be multiREEMtree.")
    }
}
