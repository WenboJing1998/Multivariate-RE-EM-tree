#' Extract the terminal nodes (leaves) of a multiREEMtree (or multitree) object
#'
#' Extract the terminal nodes (leaves) of a multiREEMtree (or multitree) object, and provide summary statistics for
#' each response variable in each node.
#'
#' @param object A multiREEMtree or multitree object.
#'
#'
...
getTerminal <- function(object){
    ...
    if(inherits(object, "multiREEMtree") | inherits(object, "multitree")){
      ...
      stop("The object should be multiREEMtree or multitree.")
    }
}
