...
#' means and sample covariance matrix, i.e. convert y to y * (cov(y)^(-1/2)) or "none" (No standardization). Default is "covariance".
...
#' @param lme.algorithm The algorithm used to fit linear mixed effect model. Either \code{"nlme"} or \code{"MCMCglmm"}.
#' @param autoCorrelation An optional string specifying the within-object correlation structure. Should be either NULL (default) or the name of
#' of one of the available corStruct classes in \link[nlme]{corClasses} (e.g., "corAR1"). This auto-correlation structure will be applied to all
...
#' to print an error message and terminate if there are any incomplete observations. Only used when \code{lme.algorithm="nlme"}.
...
#'  \item residuals - a list of the residual values of all of the levels.
...
#'  \item LogLik - The final logliklihood if \code{lme.algorithm="nlme"}.
...
#'  \item ErrorTolerance - \code{ErrorTolerance} in the function.
#'  \item Formula - \code{formula} in the function.
#'  \item Random- \code{random} in the function.
#'  \item Subset - \code{Subset} in the function.
#'  \item tree.control- \code{tree.control} in the function.
#'  \item xv - \code{tree.xv} in the function.
#'  \item xval - \code{tree.xval} in the function.
#'  \item lme.algorithm - \code{lme.algorithm} in the function.
#'  \item lme.method - \code{lme.method} in the function.
#'  \item autoCorrelation - \code{autoCorrelation} in the function.
#'  \item lme.control- \code{lme.control} in the function.
#'  \item lme.na.action - \code{lme.na.action} in the function.
#'  \item Y_original - The original values of the response variables.
#'  \item stand.y - \code{stand.y} in the function.
#' }
#'
#' @references
#'
