  # R code for the simulation studies of multivariate RE-EM tree with different
  # values of T_i. 
  ...
  multiREEMtree <- function(formula,
    ...
    if(lme.algorithm=="nlme"){
      ...
      # Replace the predicted value on each leaf with the fixed effect coefficient in the linear mixed effects model
      adjtarg <- unique(cbind(tree$where, matrix(FittedFixedMCMC, ncol=VariableNumber, byrow=F)))
...
#for(t in 1:SimulationTime)  {
   ...
   # Generate group
   group <- as.factor(c(rep(1:(I/2), each=25), rep((I/2+1):I, each=50)))
...
save(results, file=paste("results//results, I=", I, ", T=25and50, cor=", sigmab, ", noise=", sigmae, ", structure=", structure, ".RData", sep=""))
