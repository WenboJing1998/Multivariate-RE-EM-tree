  # R code for the simulation studies of multivariate RE-EM tree. 
    ...
    if(class(object)=="multiREEMtree"){
      ...
      for (i in 1:nrow(newdata)){
        if(all(is.na(newdata[i, object$Predictors]))){
          ...
          TreePrediction[i, ] <- predict(object$Tree, newdata[c(i,i), ])[1, ] # There seems to be a bug that predict in "mvpart" cannot predict a single value
        }
      }
      ...
      # if(object$stand.y == "covariance"){
      #   
      #   cov.y <- cov(object$Y_original)
      #   
      #   standard_to_original <- function(y) {
      #     y %*% sqrtm(cov.y)
      #   }
      #   
      # }else if(object$stand.y  == "marginal"){
      #   
      #   #y_mean <- apply(object$Y_original, 2, mean)
      #   
      #   y_std <- apply(object$Y_original, 2, sd)
      #   
      #   standard_to_original <- function(y) {
      #     t(t(y) * y_std)
      #   }
  ...
  is.sametree.rpart <- function(object1, object2){
     ...
     return(
        identical(as.integer(row.names(object1$frame)), as.integer(row.names(object2$frame))) &
           identical(object1$frame$var, object2$frame$var))
  }
 ...
 sigmab_seq <- c(0, 0.25, 0.5, 0.75) # Sequence of correlation between random intercept b_Y and b_W
 ...
 sigmab <- sigmab_seq[tempId2 - (tempId3 - 1) * 4 ]
 tempId4 <- ceiling(tempId3  / 4)
 Ti <- Ti_seq[tempId3 - (tempId4 - 1) * 4 ]
 structure <- structure_seq[tempId4]
...
#for(t in 1:SimulationTime)  {
   ...
   # Gnerate time and group
   # time <- rep(seq(from=1, to=5, length.out=55), G) # In each group, the time sequence is evenly distributed in [1, 5]
   
   group <- as.factor(rep(1:I, each=Ti))
 
   
   
   covb <- matrix(sigmab, nrow=VariableNumber, ncol=VariableNumber)
   diag(covb) <- 1
   
   simb <- mvrnorm(I, rep(0, VariableNumber), covb) # the i-th row of simb = (b_i0Y, b_i0W), where b_0 denotes the random intercept
   sime <- mvrnorm(N, rep(0, VariableNumber), sigmae * diag(VariableNumber)) # the i-th row of sime  =  (e_iY, e_iW)
   
   
   # Generate responses
   Responses <- FixedValue + simb[group, ] + sime
   ...
   Responses_test <- FixedValue_test + simb[group_test,  ] + e_test
   ...
   fit_multitree <- mvpart(as.formula(paste("data.matrix(train[ , ResponseVariable]) ~",  Predictors)), 
                           data=train, 
                           control=tree.control,
                           xv="1se",
                           xval=10,
                           plot.add=FALSE)
   predict_multitree <- predict(fit_multitree, newdata = test)
   ...
   recover_multitree <- is.sametree.rpart(tree_true, fit_multitree)
...
save(results, file=paste("results//results, I=", I, ", T=", Ti, ", cor=", sigmab, ", noise=", sigmae, ", structure=", structure, ".RData", sep=""))
